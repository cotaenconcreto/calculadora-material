...
with st.container(border=True):
    ...
    with col_mol:
        tipo_molde = st.selectbox("Forma de tu molde o pieza:", ["Maceta Cilíndrica / Contenedor Redondo", "Bloque Rectangular / Cuadrado Macizo", "Irregular (Cálculo por agua)"])
...
with st.container(border=True):
    ...
    if tipo_molde == "Maceta Cilíndrica / Contenedor Redondo":
        col_di, col_al_c, col_es = st.columns(3)
        with col_di:
            diametro_total = st.number_input("Diámetro exterior (cm):", min_value=0.1, value=10.0, step=0.5)
        with col_al_c:
            alto_total = st.number_input("Alto exterior (cm):", min_value=0.1, value=10.0, step=0.5)
        with col_es:
            espesor = st.number_input("Grosor del borde/pared (cm):", min_value=0.1, value=1.0, step=0.1)
            
        # Lógica matemática para descontar el hueco interno de la maceta
        if espesor >= (diametro_total / 2):
            st.error("❌ El grosor del borde no puede ser mayor o igual al radio de la maceta.")
        else:
            # Cilindro exterior completo
            radio_ext = diametro_total / 2
            volumen_exterior = math.pi * (radio_ext ** 2) * alto_total
            
            # Cilindro interior vacío (hueco)
            radio_int = radio_ext - espesor
            alto_interno = alto_total - espesor # Asumimos que el fondo tiene el mismo espesor que las paredes
            volumen_interior = math.pi * (radio_int ** 2) * alto_interno
            
            # El volumen real de concreto es la resta de ambos
            volumen_cm3 = volumen_exterior - volumen_interior
        
    elif tipo_molde == "Bloque Rectangular / Cuadrado Macizo":
...
if st.button("CALCULAR RECETA EXACTA ⚖️"):
    if volumen_cm3 <= 0:
        st.warning("⚠️ Por favor, ingresá medidas válidas.")
    else:
        ...
        st.markdown(f"### 📋 Receta para un volumen neto de concreto de {volumen_cm3:.1f} cm³")
        ...
        if tecnica == "Concreto Tradicional (1:1)":
            ...
            st.markdown("<br>", unsafe_allow_html=True)
            # Quitamos la palabra 'semillas' y dejamos solo 'Piedras'
